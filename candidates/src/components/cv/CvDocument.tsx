...
export function CvDocument({
  ...
  return (
    <div
      id={id}
      className={`overflow-hidden rounded-2xl border bg-card shadow-card ${className}`}
    >
...
function MainColumn({ profile: c }: { profile: Profile }) {
  return (
    ...
    <div className="flex flex-wrap gap-1">
      {items.map((s) => (
        <span
          ...
          className={
            variant === "brand"
              ? "rounded-full bg-brand-soft px-2 py-0.5 text-[10px] font-medium text-primary"
